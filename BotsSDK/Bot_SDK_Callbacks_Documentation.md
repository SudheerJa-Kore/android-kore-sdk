...
### 2. Show "Answered by AI" Footer
**Method:**
```java
SDKConfig.setShowAnsweredByAI(boolean show)
```
**Purpose:** Toggles the visibility of the "Answered by AI" footer and icon at the bottom of bot message bubbles.

**Functionality:**
*   **True:** Every bot response will include an "Answered by AI" label with an icon.
*   **False:** Hides the label and icon (default behavior).
*   **Automatic Localization:** The text automatically translates based on the `botLanguage` field in the bot response (e.g., Arabic, Spanish, etc.).
*   **RTL Support:** For Arabic responses, the order of the icon and text is automatically adjusted for optimal readability.

**Common Use Case:** Use this to ensure transparency with users by identifying which messages are generated by the AI model.
