...
public class BotChatActivity extends BotAppCompactActivity implements BotChatViewListener, ComposeFooterInterface, TTSUpdate, InvokeGenericWebViewInterface {
    ...
    private void applyBotLanguageIfNeeded(BotResponse botResponse) {
        if (botResponse == null || StringUtils.isNullOrEmpty(botResponse.getBotLanguage())) {
            return;
        }

        String language = botResponse.getBotLanguage().trim().toLowerCase(Locale.ROOT);
        if (!"en".equals(language) && !"ar".equals(language)) {
            return;
        }

        Locale currentLocale = SDKConfiguration.getDeviceLocale();
        if (currentLocale != null && language.equalsIgnoreCase(currentLocale.getLanguage())) {
            return;
        }

        Locale updatedLocale = Locale.forLanguageTag(language);
        SDKConfiguration.setDeviceLocale(updatedLocale);
        int layoutDirection = TextUtils.getLayoutDirectionFromLocale(updatedLocale);
        applyPreferredLayoutDirection();

        if (botContentFragment != null) {
            botContentFragment.refreshLanguageDirection(layoutDirection);
        }
        if (composeFooterFragment != null) {
            composeFooterFragment.refreshLanguageDirection(layoutDirection);
        }
        refreshHeaderLanguageDirection(getSupportFragmentManager());
    }

    private void refreshHeaderLanguageDirection(FragmentManager fragmentManager) {
    ...
    public void addMessageToAdapter(BotResponse botResponse) {
        applyBotLanguageIfNeeded(botResponse);
    ...
    protected String getPreferredString(int stringResource) {
        Locale preferredLocale = SDKConfiguration.getDeviceLocale();
        ...
        return createConfigurationContext(localizedConfiguration).getString(stringResource);
    }
    ...
    void showProgressDialogue() {
        ...
        TextView loadingText = view.findViewById(R.id.syncDialogPreparingDeviceTextView);
