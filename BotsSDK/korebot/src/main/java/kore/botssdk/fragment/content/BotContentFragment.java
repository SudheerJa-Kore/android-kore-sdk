...
import kore.botssdk.models.BrandingBodyTypingIndicatorModel;
import kore.botssdk.models.BrandingBodyModel;
import kore.botssdk.models.BrandingIconModel;
...
public class BotContentFragment extends BaseContentFragment {
    ...
    private LoadingDots botTypingStatusDots;
            ...
            if (botBrandingModel.getBody() != null) {
                ...
                if (bodyModel != null) {
                    ...
                    if (bodyModel.getTime_stamp() != null) {
                        ...
                        SDKConfiguration.setTimeStampsRequired(
                                SDKConfiguration.isTimeStampsRequired()
                                        && bodyModel.getTime_stamp().isShow()
                        );
                    }

                    applyTypingIndicatorBranding(botBrandingModel);
                    editor.apply();
                }
            }
        }
    ...
    public void showTypingStatus() {
        botTypingStatusRl.setVisibility(
                showTypingIndicator ? View.VISIBLE : View.GONE
        );
    }
    ...
    protected void initializeBotTypingStatus(View view, String mChannelIconURL) {
        ...
        botTypingStatusDots = view.findViewById(R.id.ldDots);
        botTypingStatusDots.setDotsColor(Color.parseColor(
    ...
    private void applyTypingIndicatorBranding(
            BotBrandingModel botBrandingModel
    ) {
        if (botTypingStatusIcon == null) {
            return;
        }

        BrandingBodyTypingIndicatorModel typingIndicator =
                botBrandingModel.getBody() != null
                        ? botBrandingModel.getBody().getTyping_indicator()
                        : null;
        showTypingIndicator =
                typingIndicator == null || typingIndicator.isShow();

        String icon = typingIndicator != null
                ? typingIndicator.getIcon()
                : null;
        if (StringUtils.isNullOrEmpty(icon)
                && botBrandingModel.getGeneral() != null) {
            icon = botBrandingModel.getGeneral().getBot_icon();
        }

        BrandingIconModel headerIcon =
                botBrandingModel.getHeader() != null
                        ? botBrandingModel.getHeader().getIcon()
                        : null;
        if (StringUtils.isNullOrEmpty(icon) && headerIcon != null) {
            icon = headerIcon.getIcon_url();
        }
        if (StringUtils.isNullOrEmpty(icon)) {
            icon = SDKConfiguration.BubbleColors.getIcon_url();
        }
        if (StringUtils.isNullOrEmpty(icon)) {
            icon = mChannelIconURL;
        }

        int iconResource = resolveBrandedIconResource(icon);
        String iconUrl = iconResource == -1 ? icon : null;
        botTypingStatusIcon.populateLayout(
                mBotNameInitials,
                iconUrl,
                null,
                iconResource,
                Color.parseColor(SDKConfiguration.BubbleColors.quickReplyColor),
                true
        );
        botTypingStatusDots.setDotsColor(Color.parseColor(
                sharedPreferences.getString(
    ...
    private int resolveBrandedIconResource(String icon) {
        if (BotResponse.ICON_1.equals(icon)) {
            return R.drawable.ic_icon_1;
        }
        if (BotResponse.ICON_2.equals(icon)) {
            return R.drawable.ic_icon_2;
        }
        if (BotResponse.ICON_3.equals(icon)) {
            return R.drawable.ic_icon_3;
        }
        if (BotResponse.ICON_4.equals(icon)) {
            return R.drawable.ic_icon_4;
        }
        return -1;
    }
